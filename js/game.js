...
function showMsg(m) { state.msg = m; state.msgTimer = 180; }
...
function placePiece(hIdx, ar, ac) {
  ...
  for (const [nr, nc] of cells) combat.grid[ar + nr][ac + nc] = {color: sigil.color, id: piece.id};
  let dmg = cells.length + player.bonusDmg;
...
function pickLoot(i) {
  ...
  if (next) {
    state.floor++;
    const eid = next.enemies[Math.floor(Math.random() * next.enemies.length)];
    startCombat(ENEMIES.find(e => e.id === eid));
    state.screen = 'combat';
  } else {
...
new p5(function(p) {
  ...
  function drawGrid(gx0, gy0, csz, grid, ghostCells, ghostOk) {
    ...
    for (let r = 0; r < GRID_ROWS; r++) {
      for (let c = 0; c < GRID_COLS; c++) {
        ...
        if (cell) {
          ...
          p.noFill(); p.stroke(cell.color+'55'); p.strokeWeight(1);
  ...
  function drawMiniPiece(id, rot, cx, cy, csz, sel) {
    ...
    const offX = cx - (maxC + 1) * csz / 2;
    const offY = cy - (maxR + 1) * csz / 2;
    const sigil = SIGILS.find(s => s.id === id);
    cells.forEach(([nr, nc]) => {
      dr(offX + nc*csz, offY + nr*csz, csz-1, csz-1, sel ? sigil.color : sigil.color+'aa', 2);
    });
  }
  ...
  function handLayout() {
    // Returns {hY, cardW:88, gap:8, sx} based on portrait/landscape
    if (portrait) {
  ...
  function endTurnBtnRect() {
    if (portrait) {
      const {hY, cardH} = handLayout();
      return {x: GW/2-85, y: hY+cardH+10, w:170, h:38};
    } else {
      const {hY, cardH, RX, RW} = handLayout();
      return {x: RX, y: hY+cardH+10, w: RW, h:38};
    }
  }

  function drawHands(hand, selected) {
    const hl = handLayout();
